...
const { baseURL, versions } = require("./Constants");
...
class RequestHandler {
     ...
     * @param {String} url
     * @param {body} body
     * @param {Boolean} [auth=true] Send Authentication header
     * @returns {Object | String} Parsed data
     */
    request(method, url, body, auth = true) {
        return new Promise((resolve, reject) => {
            url = new URL(`${baseURL}/api/v${versions.api}${url}`);
            ...
            .once("error", error => {
                ...
                res.on("end", () => {
                    if (res.statusCode < 200 || res.statusCode >= 300) {
                        reject(new Error(response.join("")));
                        return;
                    }
